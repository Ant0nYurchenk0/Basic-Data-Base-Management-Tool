...
void masterMarkAsDeleted(int id) {
...
struct Master* MasterTryGetValue(int id, int print) {
...
void MasterUpdate(struct Master master, int updateSlaves) {
	...
	fread(&oldMaster, MASTER_SIZE, 1, database);
	if (!updateSlaves) {
		master.slavesCount = oldMaster.slavesCount;
		master.firstSlaveAddress = oldMaster.firstSlaveAddress;
	}
...
void MasterDelete(int id) {
	...
	if (responce) {
		...
		fseek(database, indexer.address, SEEK_SET);				// Позиціонуємося за адресою запису в БД
		fread(&master, MASTER_SIZE, 1, database);
		if (master.slavesCount)								// Були поставки, видаляємо всі
		{
			...
			for (int i = 0; i < master.slavesCount; i++)
			{
				...
				fclose(slavesDb);
				SlaveDelete(slave.id, master.id);
				slavesDb = fopen(SLAVE_DATA, "r+b");
				fseek(slavesDb, slave.nextSlave, SEEK_SET);
			}
